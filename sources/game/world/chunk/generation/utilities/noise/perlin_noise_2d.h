...
public :
	...
	float					operator () (vec2 input) const
	{
		...
		float				top_left_angle = random(whole) * pi_on_2;
		float				top_right_angle = random(whole + vec2(1.0f, 0.0f)) * pi_on_2;
		float				bottom_left_angle = random(whole + vec2(0.0f, 1.0f)) * pi_on_2;
		float				bottom_right_angle = random(whole + vec2(1.0f, 1.0f)) * pi_on_2;
